...
void read_vcf(std::vector<snv>& variant_list, std::string in_vcf, std::string in_bam)
{
    ...
    while(bcf_read(vcf, hdr, rec) == 0)
    {
        ...
        if((bcf_get_info_float(hdr, rec, "AF", &vafs, &vaf_count) >= 0 ||
            bcf_get_info_float(hdr, rec, "VAF", &vafs, &vaf_count) >= 0) &&
           vaf_count > 0)
        {
            v.vaf = vafs[0];
            free(vafs);
        ...
        if((bcf_get_info_int32(hdr, rec, "MRD", &mrds, &mrd_count) >= 0 ||
            bcf_get_info_int32(hdr, rec, "VRD", &mrds, &mrd_count) >= 0) &&
           mrd_count > 0)
        {
            v.mrd = mrds[0];
            free(mrds);
        ...
        if(found_vaf_field && (v.vaf < 0.0f || v.vaf < settings.min_snv_vaf || v.vaf > settings.max_snv_vaf))
        {
            if(settings.verbose)
            {
                log_info("Skipped variant at " + v.chrom_name + ":" + std::to_string(v.pos) + " due to VAF of " +
                         std::to_string(v.vaf) + "\n");
            }
            continue;
        }
        ...
        if(found_mrd_field && v.mrd < settings.min_mrd_snv)
        {
            if(settings.verbose)
            {
                log_info("Skipped variant at " + v.chrom_name + ":" + std::to_string(v.pos) + " due to VRD of " +
                         std::to_string(v.mrd) + "\n");
            }
            continue;
        }
...
MNV_RESULT test_snv(const std::vector<snv*>& s, int num_variants, mnv* out_mnv)
{
    ...
    for(int i = 1; i < num_variants; i++)
    {
        ...
        std::set_intersection(intersect_cov.begin(), intersect_cov.end(), s[i]->covering_hashes.begin(),
                              s[i]->covering_hashes.end(), std::inserter(temp_cov, temp_cov.begin()));
...
{
    if(a->pos == b->pos || std::abs((int)a->pos - (int)b->pos) > settings.read_length)
        return 2;
...
/*
    ...
    For example, an MNV of size two turns into an MNV of size 3, if the additional SNV can make pairs with every already
   present SNV
*/
...
*/
mnv_window parse_window_to_doublets(snv_window& window, std::unordered_set<mnv, mnv_hash>& pair_cache,
                                    mnv_window& filtered, int window_id)
{
...
*/
mnv_window parse_window_to_next_phase(mnv_window& mnv_w, snv_window& snv_w,
                                      std::unordered_set<mnv, mnv_hash>& pair_cache, mnv_window& filtered,
                                      int phase_size, int window_id)
{
...
/*
    ...
    First we make all doublets, then starting from size = 3 we make every additional MNV until we reach either the SNV
   window size or the specified size.
*/
mnv_window make_all_phases_window(snv_window& window, std::unordered_set<mnv, mnv_hash>& pair_cache,
                                  mnv_window& filtered, int max_phase_size, int window_id)
{
...
mnv_window parse_window(snv_window& window, mnv_window& filtered, std::string& in_bam, int chromosome, int window_id)
{
    ...
    results = make_all_phases_window(window, cached_pairs, filtered,
                                     settings.mnv_size <= 1 ? window.size() : settings.mnv_size, window_id);
    ...
    strstr << "Finished window " << window_id << "(chr_id: " << chromosome << "). Total MNVs: " << results.size()
           << ", Total cached pairs: " << cached_pairs.size();
    log_info(strstr.str());
...
void* process_window(void* arg)
{
    ...
    {
        all_mnv_windows[window_id] =
            parse_window(all_snv_windows[window_id], all_mnv_filtered[window_id], settings.in_bam, chrom_id, window_id);
    }
...
int main(int argc, char* argv[])
{
    ...
    program.add_argument("-O", "--output-name")
        ...
        .help("The name for the output files. MNVs will be placed in <output_dir><name>.csv, the vcf in "
              "<output_dir><name>.vcf")
    ...
    program.add_argument("-Q", "--read-quality")
        .default_value(25)
        .help("Only consider reads with an equal or higher mapping quality than the specified value.")
        .store_into(settings.min_read_quality);
    program.add_argument("-V", "--verbose")
        .default_value(false)
        .help("Enables additional logging wile the program is running")
        .store_into(settings.verbose);
    program.add_argument("-T", "--threads")
        .default_value(4)
        .help("Specifies the amount of threads the program should use. A higher number means an increase in work "
              "paralellization.")
    ...
    program.add_argument("-N", "--min-vrd-mnv")
        .default_value(1)
        .help("Minimum VRD for a MNV to be considered. MNVs with a lower VRD than this will not be output.")
        .store_into(settings.min_mrd_mnv);
    program.add_argument("-Y", "--min-vaf-snv")
        .default_value(0.0f)
        .help("Minimum VAF for a SNV to be considered. SNVs with a lower VAF than this will be filtered prior to MNV "
              "analysis.")
        .store_into(settings.min_snv_vaf);
    program.add_argument("-Z", "--max-vaf-snv")
        .default_value(1.0f)
        .help("Maximum VAF for a SNV to be considered. SNVs with a higher VAF than this will be filtered prior to MNV "
              "analysis.")
    ...
    program.add_argument("-F", "--min-phi")
        .default_value(0.0)
        .help("Minimum Phi-coefficient for a pair of SNVs to be considered. Pairs with a lower Phi-coefficient than "
              "the specified value will be discarded.")
        .store_into(settings.min_phi);
    program.add_argument("-C", "--black-list")
    ...
    program.add_argument("-J", "--min-jaccard")
        .default_value(0.0)
